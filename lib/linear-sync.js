...
function generateStateChangeComment(newState, session) {
    ...
    switch (newState) {
        case 'active':
        case 'live':
            return `🟢 **Work resumed** on this issue.\n\n` +
                   `Session: \`${sessionLabel}\`\n` +
                   `Time: ${timestamp}\n\n` +
                   `_Updated automatically by OpenClaw Dashboard_`;
        
        case 'idle':
            return `⏸️ **Work paused** on this issue (session idle >30 min).\n\n` +
                   `Session: \`${sessionLabel}\`\n` +
                   `Time: ${timestamp}\n\n` +
                   `_Updated automatically by OpenClaw Dashboard_`;
        
        case 'completed':
            return `✅ **Work completed** on this issue.\n\n` +
                   `Session: \`${sessionLabel}\`\n` +
                   `Time: ${timestamp}\n\n` +
                   `_Updated automatically by OpenClaw Dashboard_`;
        
        default:
            return `📝 Session state changed to: ${newState}\n\n` +
                   `Session: \`${sessionLabel}\`\n` +
                   `Time: ${timestamp}`;
    }
}
...
async function onSessionUpdate(session) {
    ...
    if (results.updated.length > 0) {
        console.log(`[Linear] Updated ${results.updated.length} issues:`, 
