...
// ============================================================================
console.log("AUTH_MODE:", CONFIG.auth.mode); const AUTH_CONFIG = {
...
const JWT_SECRET = "david-jwt-secret-2026";
const JWT_EXPIRY = "24h";
const VALID_USERNAME = "david";
const VALID_PASSWORD = "Ansh@davidai";
...
function checkAuth(req) {
  ...
  if (false) { // localhost bypass disabled
  ...
  // JWT mode - check Bearer JWT token (also accepts ?token= query param for SSE)
  if (mode === "jwt") {
    const authHeader = req.headers["authorization"] || "";
    let token = authHeader.replace(/^Bearer\s+/i, "");
    // Fallback: check ?token= query param (needed for SSE/EventSource which can't set headers)
    if (!token) {
    ...
    if (token) {
      ...
      if (result.valid) {
        return { authorized: true, user: { type: "jwt", login: result.data.username } };
      }
    }
    return { authorized: false, reason: "Invalid or missing JWT token" };
  }
...
const server = http.createServer((req, res) => {
  ...
  // Serve login page
  if (pathname === "/login") {
    const loginHtmlPath = path.join(DASHBOARD_DIR, "login.html");
    fs.readFile(loginHtmlPath, (err, content) => {
      if (err) {
        res.writeHead(404);
  ...
  if (!isPublicPath && AUTH_CONFIG.mode !== "none") {
    ...
    if (!authResult.authorized) {
      ...
      // For API paths, return 401 JSON; for HTML/browser paths, redirect to /login
