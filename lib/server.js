...
function getOperatorBySlackId(slackId) {
    ...
    return data.operators.find(op =>
        op.id === slackId || op.metadata?.slackId === slackId
    );
}
...
function getSystemVitals() {
    ...
    try {
        ...
        if (loadMatch) {
            vitals.cpu.loadAvg = [parseFloat(loadMatch[1]), parseFloat(loadMatch[2]), parseFloat(loadMatch[3])];
        }
        ...
        try {
            ...
            try {
                const perfCores = execSync('sysctl -n hw.perflevel0.logicalcpu 2>/dev/null || echo 0', { encoding: 'utf8' });
                const effCores = execSync('sysctl -n hw.perflevel1.logicalcpu 2>/dev/null || echo 0', { encoding: 'utf8' });
            ...
            // Get CPU brand
            try {
                const brand = execSync('sysctl -n machdep.cpu.brand_string 2>/dev/null || echo ""', { encoding: 'utf8' }).trim();
                if (brand) vitals.cpu.brand = brand;
            } catch (e) {}
            
            // Get chip name for Apple Silicon
            try {
                const chip = execSync('system_profiler SPHardwareDataType 2>/dev/null | grep "Chip:" | cut -d: -f2', { encoding: 'utf8' }).trim();
        ...
        try {
            const topOutput = execSync('top -l 1 -n 0 2>/dev/null | grep "CPU usage"', { encoding: 'utf8' });
        ...
        if (isAppleSilicon) {
            ...
            try {
                const pmOutput = execSync('timeout 2 sudo -n powermetrics --samplers smc -i 1 -n 1 2>/dev/null | grep -i "die temp" | head -1 || echo ""', { encoding: 'utf8' }).trim();
            ...
            if (!vitals.temperature) {
                try {
                    const ioregRaw = execSync('ioreg -r -n AppleSmartBattery 2>/dev/null | grep Temperature || echo ""', { encoding: 'utf8' });
...
const TOPIC_PATTERNS = {
    ...
    'heartbeat': ['heartbeat', 'heartbeat_ok', 'poll', 'health check', 'ping', 'keepalive', 'monitoring'],
    ...
    // Development topics
    'coding': ['code', 'script', 'function', 'debug', 'error', 'bug', 'implement', 'refactor', 'programming'],
    'git': ['git', 'commit', 'branch', 'merge', 'push', 'pull', 'repository', 'pr', 'pull request', 'github'],
...
function getSessionTopic(sessionId) {
    ...
    try {
        ...
        let textSamples = [];
        for (const line of lines.slice(0, 30)) { // First 30 entries
            try {
...
function getMemoryStats() {
    ...
    try {
        ...
        if (fs.existsSync(memoryDir)) {
            const files = collectMemoryFiles(memoryDir, memoryDir)
...
function getTokenStats(sessions) {
    ...
    try {
        const configOutput = execSync('cat ~/.openclaw/clawdbot.json 2>/dev/null', { encoding: 'utf8' });
...
function getSessionDetail(sessionKey) {
    try {
        ...
        const toolsArray = Object.entries(tools).map(([name, count]) => ({ name, count }))
            .sort((a, b) => b.count - a.count);
        
        // Calculate last active time
        const ageMs = sessionInfo.ageMs || 0;
        const lastActive = ageMs < 60000 ? 'Just now' :
            ageMs < 3600000 ? `${Math.round(ageMs / 60000)} minutes ago` :
...
function getCerebroTopics(options = {}) {
    ...
    try {
        ...
        const topicNames = fs.readdirSync(topicsDir)
            .filter(name => {
        ...
        topicNames.forEach(name => {
            ...
            try {
                ...
                if (fs.existsSync(threadsDir)) {
                    threadCount = fs.readdirSync(threadsDir)
        ...
        if (fs.existsSync(orphansDir)) {
            try {
                result.orphans = fs.readdirSync(orphansDir)
...
function getRoutingStats(hours = 24) {
    try {
        ...
        const output = execSync(`cd "${skillDir}" && python -m llm_routing stats --hours ${hours} --json 2>/dev/null`, {
            encoding: 'utf8',
            timeout: 10000
        });
    ...
    try {
        ...
        if (output) {
            ...
            for (const s of subagentSessions) {
                ...
                // Count messages
                const messageCount = transcript.filter(e => 
                ...
                subagents.push({
                    ...
                    ageMs,
                    ageFormatted: ageMs < 60000 ? 'Just now' :
    ...
    try {
        switch (action) {
            ...
            case 'clear-stale-sessions':
                ...
                try {
                    const data = JSON.parse(output);
                    staleCount = (data.sessions || []).filter(s => 
                        s.ageMs > 24 * 60 * 60 * 1000
                    ).length;
                } catch (e) {}
...
const server = http.createServer((req, res) => {
    ...
    const isPublicPath = AUTH_CONFIG.publicPaths.some(p => pathname === p || pathname.startsWith(p + '/'));
    ...
    if (!isPublicPath && AUTH_CONFIG.mode !== 'none') {
        ...
        if (authResult.user?.login || authResult.user?.email) {
            console.log(`[AUTH] Allowed: ${authResult.user.login || authResult.user.email} (path: ${pathname})`);
        } else {
    ...
    if (pathname === '/api/status') {
        ...
        const topicId = decodeURIComponent(pathname.replace('/api/cerebro/topic/', '').replace('/status', ''));
        ...
        req.on('end', () => {
            try {
                ...
                if (!newStatus || !['active', 'resolved', 'parked'].includes(newStatus)) {
                    res.writeHead(400, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ error: 'Invalid status. Must be: active, resolved, or parked' }));
                    return;
                }
