...
function loadConfig() {
    ...
    const config = {
        ...
        paths: {
            workspace: workspace,
            memory: expandPath(process.env.OPENCLAW_MEMORY_DIR || fileConfig.paths?.memory) || 
                   path.join(workspace, 'memory'),
            state: expandPath(process.env.OPENCLAW_STATE_DIR || fileConfig.paths?.state) || 
                  path.join(workspace, 'state'),
            cerebro: expandPath(process.env.OPENCLAW_CEREBRO_DIR || fileConfig.paths?.cerebro) || 
                    path.join(workspace, 'cerebro'),
            skills: expandPath(process.env.OPENCLAW_SKILLS_DIR || fileConfig.paths?.skills) || 
                   path.join(workspace, 'skills'),
            jobs: expandPath(process.env.OPENCLAW_JOBS_DIR || fileConfig.paths?.jobs) || 
                 path.join(workspace, 'jobs'),
            logs: expandPath(process.env.OPENCLAW_LOGS_DIR || fileConfig.paths?.logs) ||
        ...
        auth: {
            ...
            allowedUsers: (process.env.DASHBOARD_ALLOWED_USERS || 
                          fileConfig.auth?.allowedUsers?.join(',') || '')
                          .split(',').map(s => s.trim().toLowerCase()).filter(Boolean),
            allowedIPs: (process.env.DASHBOARD_ALLOWED_IPS || 
                        fileConfig.auth?.allowedIPs?.join(',') || '127.0.0.1,::1')
                        .split(',').map(s => s.trim()),
